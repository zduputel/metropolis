#!/usr/bin/env python
#
# Infinite fault (strike-slip, interseismic) - Hierarchical Bayes
#
# Invert for locking depth, slip rate and data error
#
...
def calcLLK(M,data_dict):
    ...
    * data_dict: Input data
    ...
    Norm = -(np.log(M[2])+0.5*np.log(2*np.pi))*data.size

    # Residual vector
    res  = (data - calcPred(M[:2],x))/M[2] # residuals
...
def plot_result(M,mtarget,show_ini=True,title = None):
    ...
    * mtarget
    ...
    xbounds = (0.55,1.1)
    ybounds = (0.65,1.35)        
    ebounds = (mtarget[2]-0.02,mtarget[2]+0.02)
    ...
    # Data error hyper parameter
    bins = np.linspace(ebounds[0],ebounds[1],50)
    plt.figure(facecolor='w')
    ax = plt.subplot(111)
    ax.hist(M[:,2],bins=bins,normed=True)
    ax.plot([mtarget[2],mtarget[2]],[ax.get_ylim()[0],ax.get_ylim()[1]],'r',lw=4)
    ax.set_xlabel('Data uncertainty')
...
# Target model vector
mtarget = np.array([slip,lockdepth,sigdata])


## Prior Bounds (assuming uniform prior) 
prior_bounds = np.array([[0.0  ,4.0],  # Slip rate
                         [0.1  ,4.0],  # Locking depth
                         [0.001,0.5]]) # Error


## First sample in the markov chain 
m_ini = np.array((0.57,1.32,0.01))
